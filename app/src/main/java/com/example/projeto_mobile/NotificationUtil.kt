...
import com.example.projeto_mobile.BuildConfig.VERSION_CODE

object NotificationUtil {
    internal val CHANNEL_ID="1"
    fun createChanel(){
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            val contexto = AppApplication.getInstance().applicationContext
    ...
    fun create(id: Int, intent: Intent, titulo: String, texto: String){
        ...
        val builder = NotificationCompat.Builder(contexto, CHANNEL_ID).setContentIntent(p).setContentTitle(titulo)
            .setContentText(texto).setSmallIcon(R.mipmap.ic_launcher).setAutoCancel(true)
            .setVisibility(NotificationCompat.VISIBILITY_PUBLIC)

        with(NotificationManagerCompat.from(AppApplication.getInstance())){
            val n = builder.build()
            notify(id, n)
        }
    }
